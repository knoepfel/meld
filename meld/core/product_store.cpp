...
namespace meld {
  ...
  product_store::product_store(product_store const& current, bool deep, std::size_t message_id) :
  ...
  product_store_ptr
  product_store::extend(std::size_t message_id, bool deep)
  {
    return std::make_shared<product_store>(*this, deep, message_id);
  }
