...
namespace meld::detail {
  using module_creator_t = void(module_proxy<void_tag>&, configuration const&);
}

#define NARGS(...) BOOST_PP_DEC(BOOST_PP_VARIADIC_SIZE(__VA_OPT__(, ) __VA_ARGS__))

#define CREATE_1ARG(m)                                                                             \
  void create(meld::module_proxy<meld::void_tag>& m, meld::configuration const&)
#define CREATE_2ARGS(m, pset)                                                                      \
  void create(meld::module_proxy<meld::void_tag>& m, meld::configuration const& config)
